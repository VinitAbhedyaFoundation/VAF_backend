...
export class UserService {
  ...
  async createVolunteer(data: {
    ...
    try {
      ...
      // Generate the same Plogger ID used during normal signup
      const ploggerId = await this.createUserId(data.gender);
    ...
    try {
      ...
      const drives = participations.map(
        (participation) => ({
          ...
          location:
            participation.drive.driveLocation?.location ??
            null,
        }),
      );
      ...
      throw new InternalServerErrorException(
        'Failed to fetch attended drives',
      );
    }
  }
  ...
  async approveAttendance(
    ...
    try {
      ...
      // Use explicitly provided hours if available.
      // Otherwise use the total hours configured for the drive.
      const attendanceHours =
        hours !== undefined
          ? hours
          : attendance.drive.totalHours;
